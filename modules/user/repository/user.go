...
func (b *UserRepository) Create(ctx context.Context, user entity.User) (int, error) {
	tx, err := b.Db.Begin()
	if err != nil {
		return 0, err
	}
...
func (b *UserRepository) FindAll(ctx context.Context, option []filter.Field) ([]entity.User, error) {
	tx, _ := b.Db.Begin()
	...
	if errQuery != nil {
		return nil, utils.NewAppError(errQuery, "Can't get query result", "user repository")
	}
	...
	for result.Next() {
		user := entity.User{BaseEntity: &utils.BaseEntity{}}
...
func (b *UserRepository) FindOne(ctx context.Context, userId int) (entity.User, error) {
	tx, _ := b.Db.Begin()
	defer utils.CommitOrRollback(tx)
	...
	for result.Next() {
		err := result.Scan(&user.Id, &user.CreatedAt, &user.UpdatedAt, &user.Username, &user.Role, &user.Profile.FirstName, &user.Profile.MiddleName, &user.Profile.LastName, &user.Profile.Age, &user.Profile.Head)
		return user, utils.NewAppError(err, "", "")

	}
	return user, nil
}

func (b *UserRepository) Update(ctx context.Context, user entity.User, userId int) (entity.User, error) {
	tx, _ := b.Db.Begin()
	...
	_, _ = tx.ExecContext(ctx, SQL, userId, user.Username, user.Password, user.Role)
	foundedUser, err := b.FindOne(ctx, userId)
	if err != nil {
...
func (b *UserRepository) Delete(ctx context.Context, userId int) error {
	tx, err := b.Db.Begin()
	defer utils.CommitOrRollback(tx)
	SQL := "DELETE FROM users  WHERE id = $1"
	_, err = tx.ExecContext(ctx, SQL, userId)
	if err != nil {
		return utils.NewAppError(err, "Can't exec context", "Delete user")
	}
	return nil

}

func (b *UserRepository) FindByUsername(ctx context.Context, username string) (entity.User, error) {
	tx, _ := b.Db.Begin()
	defer utils.CommitOrRollback(tx)
	SQL := "SELECT id, username, password FROM users WHERE username = $1"
	result, _ := tx.QueryContext(ctx, SQL, username)
	el := entity.User{BaseEntity: &utils.BaseEntity{}}
	for result.Next() {
		err := result.Scan(&el.Id, &el.Username, &el.Password)
		return el, utils.NewAppError(err, "Can't get result", "FindByUsername")
